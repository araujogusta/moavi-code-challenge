...
class GetMarkingTest(TestCase):
    def setUp(self):
        ...
        self.marking_import = MarkingImport.objects.create(
            filename="test_file.csv", markings_count=1
        )
    ...
    def test_markings_ordered_by_date_and_hour(self):
        ...
        self.create_marking(
            date=now.date(), hour=(now - timedelta(hours=1)).time(), employee_id=2
        )
        self.create_marking(
            date=now.date() - timedelta(days=1), hour=now.time(), employee_id=3
        )
...
class GetChartDataTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.marking_import = MarkingImport.objects.create(
            filename="dummy.csv", markings_count=0
        )
